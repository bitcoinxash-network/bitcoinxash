...
*Setup instructions for a gitian build of BCX using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the BCX
...
Clone the git repositories for bcx and gitian and then checkout the bcx version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the bcx repository
...
To build BCX (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the bcx repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/bitcoinxash/.git/
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/bcx-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/bcx-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/bcx-build.assert
```
...
[bcx/gitian.sigs](https://github.com/bitcoinxash/gitian.sigs/) repository, or if that's not possible to create a pull
request.
