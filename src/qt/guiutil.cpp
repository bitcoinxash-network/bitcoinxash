...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a BCX address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
...
{
    // return if URI is not valid or is no bcx: URI
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient* out)
{
    // Convert bcx:// to bcx:
    //
    //    Cannot handle this later, because bcx:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open bcx.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "BCX.lnk";
}
...
{
    return GetAutostartDir() / "bcx.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a bcx.desktop file to the autostart directory:
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the bcx app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add bcx app to startup item list
